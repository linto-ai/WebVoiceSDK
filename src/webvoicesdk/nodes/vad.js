...
import { RNNOISE_SAMPLE_LENGTH, Rnnoise } from "../rnnoise/index.js"
...
const handler = function (nodeEvent) {
  ...
  for (
    ;
    i + RNNOISE_SAMPLE_LENGTH < completeInData.length;
    i += RNNOISE_SAMPLE_LENGTH
  ) {
    ...
    if (this.activations.length == this.options.numActivations)
    ...
    if (
      vadScore < this.options.threshold &&
