...
export default class Mic extends Node {
  ...
  async start(deviceId = null) {
    if (this.hookedOn)
      throw new NodeError(`Microphone is already started, call stop() first`)
    ...
    this.micFrameGenerator = this.audioContext.createScriptProcessor(
    ...
    if (this.hookedOn) {
      if (this.stream) {
        this.stream.getTracks().map((track) => {
          return track.readyState === "live" && track.kind === "audio"
            ? track.stop()
            : false
        })
      }
