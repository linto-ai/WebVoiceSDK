...
export default class Node extends EventTarget {
  ...
  async start(node) {
    if (this.hookedOn)
      throw new NodeError(
        `node ${this.type} is already hooked on ${this.hookedOn}, call stop() first`,
      )
    if (!node)
      throw new NodeError(`${this.type} requires a node argument to hook on`)
    if (node && !this.hookableOnNodeTypes.includes(node.type))
      throw new NodeError(`${this.type} node cannot hook on ${node.type}`)
    this.startWorker()
    this.hookedOn = node
    this.resume()
  }
  ...
  pause() {
    if (this.hookedOn && this.status == "emitting") {
      if (this.type != "mic")
        this.hookedOn.removeEventListener(this.hookedOn.event, this.handler)
      this.status = "non-emitting"
    }
  }
  ...
  resume() {
    ...
    if (this.hookedOn && this.status == "non-emitting") {
      if (this.type != "mic")
        this.hookedOn.addEventListener(this.hookedOn.event, this.handler)
