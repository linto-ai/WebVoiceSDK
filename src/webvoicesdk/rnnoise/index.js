...
export class Rnnoise {
  ...
  calculateAudioFrameVAD(pcmFrame) {
    if (this.destroyed) {
      throw new Error(
        "RnnoiseProcessor instance is destroyed, please create another one!",
      )
    }
    ...
    if (pcmFrameLength !== RNNOISE_SAMPLE_LENGTH) {
      throw new Error(
        `Rnnoise can only process PCM frames of 480 samples! Input sample was:${pcmFrameLength}`,
      )
    }

    this.convertTo16BitPCM(pcmFrame)
    this.copyPCMSampleToWasmBuffer(pcmFrame)
    return this.wasmInterface._rnnoise_process_frame(
      this.context,
      this.wasmPcmOutput,
      this.wasmPcmInput,
    )
