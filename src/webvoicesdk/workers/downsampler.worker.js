...
function process(audioFrame) {
  ...
  while (
    (inputBuffer.length * targetSampleRate) / nativeSampleRate >
    targetFrameSize
  ) {
    let outputFrame
    Int16Convert
      ? (outputFrame = new Int16Array(targetFrameSize))
      : (outputFrame = new Float32Array(targetFrameSize))
    let sum = 0
    let num = 0
    let outputIndex = 0
    let inputIndex = 0
    while (outputIndex < targetFrameSize) {
      ...
      while (
        ...
        Math.min(
          inputBuffer.length,
          ((outputIndex + 1) * nativeSampleRate) / targetSampleRate,
        )
      ) {
